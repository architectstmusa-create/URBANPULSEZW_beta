...
let buyScore = Math.round(
  ...
  developmentScore * 1.1 +
  infrastructureScore * 1.2 +
  marketScore * 1.4 +
  premiumBonus
);
...
let savedReports =
  JSON.parse(localStorage.getItem("urbanpulse_reports")) || [];

/* Prevent duplicates if the user refreshes */
savedReports = savedReports.filter(
  report =>
    !(
      report.location === reportData.location &&
      report.coordinates === reportData.coordinates
    )
);
...
savedReports = savedReports.slice(0, 20);
